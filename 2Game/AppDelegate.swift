...
//  2Game
//
//  Created by Волохин Александр Сергеевич on 03/03/2018.
//  Copyright © 2018 academboys. All rights reserved.
//
...
class AppDelegate: UIResponder, UIApplicationDelegate {
	...
	public func application(_ application: UIApplication, willFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
		...
		if name != nil && avatarIndex != nil && classIndex != nil {
			page = UserVC()
		} else {
			page = NameVC()
		}

		let navcon = UINavigationController(rootViewController: page)
		self.window?.rootViewController = navcon
		self.window?.makeKeyAndVisible()

		let s = GameService.shared
		s.start()
